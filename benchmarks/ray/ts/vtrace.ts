...
class Vec3 {
  ...
  refract(normal: Vec3, etaiOverEtat: number): Vec3 {
    ...
    const rOutPerp = this.add(normal.mulScalar(cosTheta)).mulScalar(
      etaiOverEtat,
    );
    const rOutParallel = normal.mulScalar(
      -Math.sqrt(Math.abs(1 - rOutPerp.lenSquared())),
    );
...
class Metal implements Material {
  ...
  scatter(
    ...
    reflected = reflected
      .unitVector()
      .add(Vec3.randomUnitVector(rng).mulScalar(this.fuzz));
...
class Camera {
  ...
  constructor(readonly options: CameraOptions) {
    this.imageHeight = Math.max(
      1,
      Math.trunc(options.imageWidth / options.aspectRatio),
    );
    ...
    const viewportWidth =
      viewportHeight * (options.imageWidth / this.imageHeight);
    ...
    this.pixel00Loc = viewportUpperLeft.add(
      this.pixelDeltaU.add(this.pixelDeltaV).mulScalar(0.5),
    );
    this.pixelSamplesScale = 1 / options.samplesPerPixel;

    const defocusRadius =
      options.focusDist * Math.tan(degreesToRadians(options.defocusAngle / 2));
  ...
  async render(
    ...
    for (let j = 0; j < this.imageHeight; j += 1) {
      ...
      for (let i = 0; i < this.options.imageWidth; i += 1) {
        ...
        for (
          let sample = 0;
          sample < this.options.samplesPerPixel;
          sample += 1
        ) {
  ...
  private getRay(i: number, j: number, rng: RandomSource): Ray {
    ...
    const rayOrigin =
      this.options.defocusAngle <= 0
        ? this.center
        : this.defocusDiskSample(rng);
  ...
  if (world.hit(ray, RAY_T, rec)) {
    ...
    if (rec.mat.scatter(ray, rec, target, rng)) {
      return target.attenuation.mulVec(
        rayColor(target.scattered, depth - 1, world, rng),
      );
    }
    return new Vec3(0, 0, 0);
  }
...
async function renderVTrace(options: RenderOptions = {}): Promise<void> {
  ...
  world.add(
    new Sphere(new Vec3(-4, 1, 0), 1, new Lambertian(new Vec3(0.4, 0.3, 0.2))),
  );
  world.add(
  ...
  const camera = new Camera({
    ...
    samplesPerPixel: options.samplesPerPixel ?? 20,
